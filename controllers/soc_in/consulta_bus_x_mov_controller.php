...
function paridad($datosDecode, $datos_bus) {
    if ($datosDecode[0]->fecha == 0) {
        if ($datosDecode[0]->in_fecha == 0) {
            echo $datos_bus; //primer registro del bus valido
        } else {
            echo 1; //ultimo registro del bus soc_in no cumple con la paridad
        }
    } elseif ($datosDecode[0]->fecha == 0) {
